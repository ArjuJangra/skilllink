    ...
    <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
      ...
      <nav class="hidden sm:flex items-center gap-3">
        ...
        <router-link :to="{ path: '/home', query: { disableBooking: true } }">
          <button class="btn-secondary flex items-center gap-2 px-4 py-2 text-gray-600 font-bold rounded-lg hover:ring-1 hover:text-[#0073b1] hover:ring-[#0073b1] transition">
            <!-- SVG icon -->
            Book Service
          </button>
        </router-link>
        <router-link to="/login">
          <button class="btn-primary px-4 py-2 bg-[#0073b1] text-white font-semibold rounded-lg hover:bg-[#005f91] transition">Login/Sign Up</button>
        </router-link>
      </nav>

      <!-- Mobile Hamburger -->
      <button class="sm:hidden flex flex-col gap-1 focus:outline-none" @click="isOpen = !isOpen">
        <span :class="{'rotate-45 translate-y-1.5': isOpen}" class="block w-6 h-0.5 bg-gray-800 transition-transform"></span>
      ...
      <div v-if="isOpen" class="fixed inset-0 z-50 bg-white flex flex-col">
        ...
        <div class="w-64 h-full bg-white shadow-lg flex flex-col px-6 py-6 space-y-6 relative">
          ...
          <router-link to="/login" @click="closeMenu">
            <button class="w-full px-4 py-2 bg-[#0073b1] text-white font-semibold rounded-lg hover:bg-[#005f91] transition">Login/Sign Up</button>
          </router-link>
        </div>
      </div>
    </transition>
