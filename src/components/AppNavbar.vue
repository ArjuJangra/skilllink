<template>
  ...
  <header v-if="props.mode !== 'homelogged'"
    ...
    <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
      ...
      <!-- Mobile Book Service -->
      <div class="sm:hidden flex justify-end flex-1" v-if="!props.hideExtras">
      ...
      <nav class="hidden sm:flex items-center gap-3">
        ...
        <template v-else-if="showButton === 'login'">
          <router-link to="/login" class="ml-auto">
            <button
              class="btn-primary px-4 py-2 bg-[#0073b1] text-white font-semibold rounded-lg hover:bg-[#005f91] transition">
              Login / Sign Up
            </button>
          </router-link>
        </template>
    ...
    <!-- Mobile Drawer -->
    <transition name="slide" enter-active-class="transition-transform duration-300 ease-out" enter-from-class="translate-x-full"
